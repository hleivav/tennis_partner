            ...
            <nav id="main-nav" className="site-nav open" aria-label="Huvudnavigering" style={{zIndex: 1010, position: 'fixed', left: 0, right: 0, top: 64, margin: '0 auto', maxWidth: 320}}>
              ...
              {user && hasNewInvitations && (
                <button
                  ...
                  onClick={e => { e.stopPropagation(); setOpen(false); navigate('/invitations'); }}
              ...
              {user ? (
                <>
                  ...
                  <button onClick={() => { setOpen(false); handleLogout(); }} style={{ background: '#fff', color: '#1A2A80', border: '1px solid #ccc', borderRadius: 6, padding: '6px 12px', fontWeight: 600, cursor: 'pointer' }}>Logga ut</button>
                </>
              ) : (
