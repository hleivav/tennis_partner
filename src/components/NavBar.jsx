  ...
  return (
    <header className="site-header">
      <div className="inner">
        ...
        <nav id="main-nav" className={`site-nav ${open ? 'open' : ''}`} aria-label="Huvudnavigering" style={open ? {zIndex: 20, position: 'relative'} : {}}>
          <Link to="/" onClick={() => { setOpen(false); navigate('/'); }}>Hem</Link>
          {user && user.role === 'SUPERADMIN' && (
            <Link to="/admin" onClick={() => { setOpen(false); navigate('/admin'); }}>Admin</Link>
          )}
          {!user && (
            <Link to="/register" onClick={() => { setOpen(false); navigate('/register'); }}>Registrering</Link>
          )}
          <Link to="/search" onClick={() => { setOpen(false); navigate('/search'); }}>Sök medspelare</Link>
          <Link to="/invitations" onClick={() => { setOpen(false); navigate('/invitations'); }} style={{ position: 'relative', display: 'inline-block' }}>
            Inbjudan
          </Link>
          ...
          {user ? (
            <>
              <Link to="/profile" onClick={() => { setOpen(false); navigate('/profile'); }}>{user.name}</Link>
